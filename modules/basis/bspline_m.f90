module BSpline_m
  ...
  type BSplineBasis_t
    real ( kind = real_w ), allocatable :: knots_( : )
    integer ( kind = int_w ) :: index_
    integer ( kind = int_w ) :: order_
...
contains
  pure function N( this, u ) result ( N_i_p )
    ...
    real( kind = real_w ), intent( in ) :: u
    real( kind = real_w ) :: N_i_p
  ...
  pure recursive function computeBasis( knots, index, order, u ) result( N_i_p )
    real ( kind = real_w ), intent( in ) :: knots( : )
    integer ( kind = int_w ), intent( in ) :: index
    integer ( kind = int_w ), intent( in ) :: order
    real ( kind = real_w ), intent( in ) :: u
    real ( kind = real_w ) :: N_i_p
    real ( kind = real_w ) :: left
    real ( kind = real_w ) :: right
